...
inline void render_source_sample(int32_t &outL, int32_t &outR) {
  ...
  // UN echantillon a la fois, et pas de travail par lots.
  //
  // J'avais ajoute un lot de 64 echantillons pour amortir l'appel. C'etait
  // FAUX : un bloc entre deux tics fait souvent bien moins de 64 echantillons,
  // et jeter le reste du lot ne « derembobine » pas la puce — elle avait deja
  // avance de 64 tics. Elle tournait donc plusieurs fois trop vite tout en
  // n'etant echantillonnee que par bribes, ce qui donnait un son de modulation
  // en anneau, façon voix de Dalek. Le lot ne rapportait que 2 %.
  if (g_ladder)
  ...
  // morceau suivant.
  // Niveau general a 1,0, comme sur l'iPad.
  //
  // Je l'avais divise par deux pour verifier si les « flatulences » venaient
  // d'un ecretage. MESURE : sur ce morceau, a niveau plein, la crete atteint
  // -1,1 dBFS et AUCUN echantillon ne sature. Il n'y avait donc rien a corriger
  // ici, et le demi-niveau ne faisait qu'enlever 6 dB a tout le monde.
